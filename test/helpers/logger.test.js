...
describe('Logger Helper', () => {
  ...
  it('Should call logger with default options', () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(/no options/))
  })
  ...
  it('Should call logger with options.level = debug and verbose set', () => {
    jest.spyOn(console, 'debug').mockImplementation(() => {})
    logger.log('message with debug level and verbose', { level: 'debug', args: { verbose: true } })
    expect(console.debug).toHaveBeenCalledWith(expect.stringMatching(/debug level and verbose/))
  })
